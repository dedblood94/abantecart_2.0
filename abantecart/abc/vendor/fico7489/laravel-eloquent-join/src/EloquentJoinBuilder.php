...
class EloquentJoinBuilder extends Builder
{
    ...
    public function orderByJoin($column, $direction = 'asc', $aggregateMethod = null)
    {
        ...
        if (false !== $dotPos) {
            ...
            $sortAlias = substr($baseColumn, $dotPos+1, strlen($baseColumn));
    ...
    private function performJoin($relations, $leftJoin = null)
    {
        ...
        $baseTable = $baseModel->getFullTableName();
        $basePrimaryKey = $baseModel->getKeyName();

        $deleted_at = 'deleted_at';
        if ($baseModel::DELETED_AT) {
            $deleted_at = $baseModel::DELETED_AT;
        }

        $currentModel      = $baseModel;
        $currentTableAlias = $baseTable;

        $currentTableAliasWithoutPrefix = $baseModel->getTable();
        ...
        foreach ($relations as $relation) {
            ...
            $relatedTable      = $relatedModel->getFullTableName();
            $relatedTableAlias = $this->useTableAlias ? uniqid() : $relatedTable;
            $relatedTableAliasWithoutPrefix = $this->useTableAlias ? uniqid() : $relatedModel->getTable();
            ...
            if ($this->appendRelationsCount) {
                $this->selectRaw('COUNT('.$relatedTableAliasWithoutPrefix.'.'.$relatedPrimaryKey.') as '.$relationAccumulatedString.'_count');
            }

            if (!in_array($relationAccumulatedString, $this->joinedTables)) {
                $joinQuery =  $relatedTableAliasWithoutPrefix.($this->useTableAlias ? ' as '. $relatedTableAliasWithoutPrefix : '');
                if ($relatedRelation instanceof BelongsToJoin) {
                    ...
                    $this->$joinMethod($joinQuery, function ($join)
                    use ($relatedRelation, $relatedTableAlias, $relatedKey, $currentTableAlias, $ownerKey, $deleted_at) {
                        $join->on($relatedTableAlias.'.'.$ownerKey, '=', $currentTableAlias.'.'.$relatedKey);

                        $this->joinQuery($join, $relatedRelation, $relatedTableAlias, $deleted_at);
                    });
                    ...
                    $this->$joinMethod($joinQuery, function ($join) use
                    ($relatedRelation, $relatedTableAlias, $relatedKey, $currentTableAlias, $localKey, $relatedTableAliasWithoutPrefix, $currentTableAliasWithoutPrefix, $deleted_at) {
                        $join->on($relatedTableAliasWithoutPrefix.'.'.$relatedKey, '=', $currentTableAliasWithoutPrefix.'.'.$localKey);
                        $this->joinQuery($join, $relatedRelation, $relatedTableAliasWithoutPrefix, $deleted_at);
                    });
                } else {
        ...
        if (!$this->selected && count($relations) > 1) {
            ...
            $this->groupBy($currentTableAliasWithoutPrefix.'.'.$basePrimaryKey);
        }

        return $currentTableAlias.'.'.$column;
    }

    private function joinQuery($join, $relation, $relatedTableAlias, $deleted_at)
    {
        ...
        if (isset($relationBuilder->relationClauses)) {
            foreach ($relationBuilder->relationClauses as $clause) {
                foreach ($clause as $method => $params) {
                    $this->applyClauseOnRelation($join, $method, $params, $relatedTableAlias, $deleted_at);
                }
            }
        }
        ...
        foreach ($relationBuilder->scopes as $scope) {
            if ($scope instanceof SoftDeletingScope) {
                $this->applyClauseOnRelation($join, 'withoutTrashed', [], $relatedTableAlias, $deleted_at);
            } else {
    ...
    private function applyClauseOnRelation(JoinClause $join, string $method, array $params, string $relatedTableAlias, string $deleted_at)
    {
